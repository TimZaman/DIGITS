    ...
    def create_model(self, obj_UserModel, stage_scope):
        ...
        if len(available_devices) == 1:
            ...
            with tf.name_scope('parallelize'):
                # Split them up
                batch_x_split = tf.split(axis=0, num_or_size_splits=len(available_devices), value=self.dataloader.batch_x, name='split_batch')
                if self.stage != digits.STAGE_INF: # Has no labels
                    batch_y_split = tf.split(axis=0, num_or_size_splits=len(available_devices), value=self.dataloader.batch_y, name='split_batch')
